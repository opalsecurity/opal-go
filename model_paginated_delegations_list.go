/*
Opal API

The Opal API is a RESTful API that allows you to interact with the Opal Security platform programmatically.

API version: 1.0
Contact: hello@opal.dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package opal

import (
	"encoding/json"
)

// checks if the PaginatedDelegationsList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PaginatedDelegationsList{}

// PaginatedDelegationsList A list of delegations for your organization.
type PaginatedDelegationsList struct {
	// The delegations in the result set.
	Results []Delegation `json:"results,omitempty"`
	// The cursor with which to continue pagination if additional result pages exist.
	Next *string `json:"next,omitempty"`
	// The cursor used to obtain the current result page.
	Previous *string `json:"previous,omitempty"`
	// The total number of items in the result set.
	TotalCount *int32 `json:"total_count,omitempty"`
}

// NewPaginatedDelegationsList instantiates a new PaginatedDelegationsList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPaginatedDelegationsList() *PaginatedDelegationsList {
	this := PaginatedDelegationsList{}
	return &this
}

// NewPaginatedDelegationsListWithDefaults instantiates a new PaginatedDelegationsList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPaginatedDelegationsListWithDefaults() *PaginatedDelegationsList {
	this := PaginatedDelegationsList{}
	return &this
}

// GetResults returns the Results field value if set, zero value otherwise.
func (o *PaginatedDelegationsList) GetResults() []Delegation {
	if o == nil || IsNil(o.Results) {
		var ret []Delegation
		return ret
	}
	return o.Results
}

// GetResultsOk returns a tuple with the Results field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaginatedDelegationsList) GetResultsOk() ([]Delegation, bool) {
	if o == nil || IsNil(o.Results) {
		return nil, false
	}
	return o.Results, true
}

// HasResults returns a boolean if a field has been set.
func (o *PaginatedDelegationsList) HasResults() bool {
	if o != nil && !IsNil(o.Results) {
		return true
	}

	return false
}

// SetResults gets a reference to the given []Delegation and assigns it to the Results field.
func (o *PaginatedDelegationsList) SetResults(v []Delegation) {
	o.Results = v
}

// GetNext returns the Next field value if set, zero value otherwise.
func (o *PaginatedDelegationsList) GetNext() string {
	if o == nil || IsNil(o.Next) {
		var ret string
		return ret
	}
	return *o.Next
}

// GetNextOk returns a tuple with the Next field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaginatedDelegationsList) GetNextOk() (*string, bool) {
	if o == nil || IsNil(o.Next) {
		return nil, false
	}
	return o.Next, true
}

// HasNext returns a boolean if a field has been set.
func (o *PaginatedDelegationsList) HasNext() bool {
	if o != nil && !IsNil(o.Next) {
		return true
	}

	return false
}

// SetNext gets a reference to the given string and assigns it to the Next field.
func (o *PaginatedDelegationsList) SetNext(v string) {
	o.Next = &v
}

// GetPrevious returns the Previous field value if set, zero value otherwise.
func (o *PaginatedDelegationsList) GetPrevious() string {
	if o == nil || IsNil(o.Previous) {
		var ret string
		return ret
	}
	return *o.Previous
}

// GetPreviousOk returns a tuple with the Previous field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaginatedDelegationsList) GetPreviousOk() (*string, bool) {
	if o == nil || IsNil(o.Previous) {
		return nil, false
	}
	return o.Previous, true
}

// HasPrevious returns a boolean if a field has been set.
func (o *PaginatedDelegationsList) HasPrevious() bool {
	if o != nil && !IsNil(o.Previous) {
		return true
	}

	return false
}

// SetPrevious gets a reference to the given string and assigns it to the Previous field.
func (o *PaginatedDelegationsList) SetPrevious(v string) {
	o.Previous = &v
}

// GetTotalCount returns the TotalCount field value if set, zero value otherwise.
func (o *PaginatedDelegationsList) GetTotalCount() int32 {
	if o == nil || IsNil(o.TotalCount) {
		var ret int32
		return ret
	}
	return *o.TotalCount
}

// GetTotalCountOk returns a tuple with the TotalCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaginatedDelegationsList) GetTotalCountOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalCount) {
		return nil, false
	}
	return o.TotalCount, true
}

// HasTotalCount returns a boolean if a field has been set.
func (o *PaginatedDelegationsList) HasTotalCount() bool {
	if o != nil && !IsNil(o.TotalCount) {
		return true
	}

	return false
}

// SetTotalCount gets a reference to the given int32 and assigns it to the TotalCount field.
func (o *PaginatedDelegationsList) SetTotalCount(v int32) {
	o.TotalCount = &v
}

func (o PaginatedDelegationsList) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PaginatedDelegationsList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Results) {
		toSerialize["results"] = o.Results
	}
	if !IsNil(o.Next) {
		toSerialize["next"] = o.Next
	}
	if !IsNil(o.Previous) {
		toSerialize["previous"] = o.Previous
	}
	if !IsNil(o.TotalCount) {
		toSerialize["total_count"] = o.TotalCount
	}
	return toSerialize, nil
}

type NullablePaginatedDelegationsList struct {
	value *PaginatedDelegationsList
	isSet bool
}

func (v NullablePaginatedDelegationsList) Get() *PaginatedDelegationsList {
	return v.value
}

func (v *NullablePaginatedDelegationsList) Set(val *PaginatedDelegationsList) {
	v.value = val
	v.isSet = true
}

func (v NullablePaginatedDelegationsList) IsSet() bool {
	return v.isSet
}

func (v *NullablePaginatedDelegationsList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePaginatedDelegationsList(val *PaginatedDelegationsList) *NullablePaginatedDelegationsList {
	return &NullablePaginatedDelegationsList{value: val, isSet: true}
}

func (v NullablePaginatedDelegationsList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePaginatedDelegationsList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


