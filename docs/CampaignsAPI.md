...
[**GetCampaignItems**](CampaignsAPI.md#GetCampaignItems) | **Get** /campaigns/{campaign_id}/items | List campaign items
[**GetCampaignViewerItems**](CampaignsAPI.md#GetCampaignViewerItems) | **Get** /campaigns/{campaign_id}/viewer-items | List viewer campaign items
...
> PaginatedCampaignItemsList GetCampaignItems(ctx, campaignId).Cursor(cursor).PageSize(pageSize).Statuses(statuses).ReviewerUserId(reviewerUserId).Unassigned(unassigned).Search(search).Execute()
...
func main() {
	...
	pageSize := int32(200) // int32 | Number of results to return per page. Default is 200; maximum is 500 (the campaign-items list cap).  (optional)
	statuses := []openapiclient.CampaignItemStatusEnum{openapiclient.CampaignItemStatusEnum("PENDING")} // []CampaignItemStatusEnum | Filter by derived item status. When multiple values are provided, items matching any status are returned (OR).  (optional)
	reviewerUserId := "32acc112-21ff-4669-91c2-21e27683eaa1" // string | Restrict to items assigned to this reviewer user ID. (optional)
	unassigned := true // bool | When true, restrict to items with no reviewer assigned. (optional)
	search := "search_example" // string | Case-insensitive search over principal name, asset name, and reviewer name.  (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.CampaignsAPI.GetCampaignItems(context.Background(), campaignId).Cursor(cursor).PageSize(pageSize).Statuses(statuses).ReviewerUserId(reviewerUserId).Unassigned(unassigned).Search(search).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CampaignsAPI.GetCampaignItems``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetCampaignItems`: PaginatedCampaignItemsList
	fmt.Fprintf(os.Stdout, "Response from `CampaignsAPI.GetCampaignItems`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetCampaignItemsRequest struct via the builder pattern
 ...
 **pageSize** | **int32** | Number of results to return per page. Default is 200; maximum is 500 (the campaign-items list cap).  | 
 **statuses** | [**[]CampaignItemStatusEnum**](CampaignItemStatusEnum.md) | Filter by derived item status. When multiple values are provided, items matching any status are returned (OR).  | 
 **reviewerUserId** | **string** | Restrict to items assigned to this reviewer user ID. | 
 **unassigned** | **bool** | When true, restrict to items with no reviewer assigned. | 
 **search** | **string** | Case-insensitive search over principal name, asset name, and reviewer name.  | 
...
> PaginatedViewerCampaignItemsList GetCampaignViewerItems(ctx, campaignId).Cursor(cursor).PageSize(pageSize).SortField(sortField).SortDirection(sortDirection).Statuses(statuses).AccessLevelNames(accessLevelNames).PrincipalIds(principalIds).EntityIds(entityIds).ItemIds(itemIds).AiBucketId(aiBucketId).Execute()
...
func main() {
	campaignId := "f454d283-ca87-4a8a-bdbb-df212eca5353" // string | The ID of the campaign.
	cursor := "cD0yMDIxLTAxLTA2KzAzJTNBMjQlM0E1My40MzQzMjYlMkIwMCUzQTAw" // string | The pagination cursor value. (optional)
	pageSize := int32(200) // int32 | Number of results to return per page. Default is 200; maximum is 500.  (optional)
	sortField := openapiclient.ViewerCampaignItemSortFieldEnum("PRINCIPAL_NAME") // ViewerCampaignItemSortFieldEnum | Sort by. Omit to sort by created_at.  (optional)
	sortDirection := openapiclient.SortDirectionEnum("ASC") // SortDirectionEnum | Sort direction. (optional)
	statuses := []openapiclient.ViewerCampaignItemStatusEnum{openapiclient.ViewerCampaignItemStatusEnum("APPROVED")} // []ViewerCampaignItemStatusEnum | Filter by review status. When multiple values are provided, items matching any status are returned (OR).  (optional)
	accessLevelNames := []string{"Inner_example"} // []string | Filter by access level name. (optional)
	principalIds := []string{"Inner_example"} // []string | Filter by principal ID. (optional)
	entityIds := []string{"Inner_example"} // []string | Filter by entity ID. (optional)
	itemIds := []string{"Inner_example"} // []string | Filter to specific campaign item review IDs. (optional)
	aiBucketId := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string | Restrict to items in this AI-suggestion bucket. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.CampaignsAPI.GetCampaignViewerItems(context.Background(), campaignId).Cursor(cursor).PageSize(pageSize).SortField(sortField).SortDirection(sortDirection).Statuses(statuses).AccessLevelNames(accessLevelNames).PrincipalIds(principalIds).EntityIds(entityIds).ItemIds(itemIds).AiBucketId(aiBucketId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CampaignsAPI.GetCampaignViewerItems``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetCampaignViewerItems`: PaginatedViewerCampaignItemsList
	fmt.Fprintf(os.Stdout, "Response from `CampaignsAPI.GetCampaignViewerItems`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetCampaignViewerItemsRequest struct via the builder pattern
 ...
 **cursor** | **string** | The pagination cursor value. | 
 **pageSize** | **int32** | Number of results to return per page. Default is 200; maximum is 500.  | 
 **sortField** | [**ViewerCampaignItemSortFieldEnum**](ViewerCampaignItemSortFieldEnum.md) | Sort by. Omit to sort by created_at.  | 
 **sortDirection** | [**SortDirectionEnum**](SortDirectionEnum.md) | Sort direction. | 
 **statuses** | [**[]ViewerCampaignItemStatusEnum**](ViewerCampaignItemStatusEnum.md) | Filter by review status. When multiple values are provided, items matching any status are returned (OR).  | 
 **accessLevelNames** | **[]string** | Filter by access level name. | 
 **principalIds** | **[]string** | Filter by principal ID. | 
 **entityIds** | **[]string** | Filter by entity ID. | 
 **itemIds** | **[]string** | Filter to specific campaign item review IDs. | 
 **aiBucketId** | **string** | Restrict to items in this AI-suggestion bucket. | 
